...
a external metric like `requests-per-second` might be pretty handy, see the example:
...
autoscaler:
  ...
  - type: requests-per-second
    average: 30
    hostname: 'example.com;foo.bar.baz'
    weight: 100
```
Note that `weight` field is not mandatory, in case it is not specified it will be assumed the value is 100%, and `hostname`
accepts multiple hostnames using `;` as separator the result is the sum of average RPS of each hostname will be compared to
the target specified by `average` field.
