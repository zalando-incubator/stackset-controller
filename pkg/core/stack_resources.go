...
func (sc *StackContainer) stackHostnames(hostnames []string, overrides *zv1.StackIngressRouteGroupOverrides) ([]string, error) {
...
func stackIngressRouteGroupAnnotations(baseMeta zv1.EmbeddedObjectMetaWithAnnotations, overrides *zv1.StackIngressRouteGroupOverrides) map[string]string {
	if overrides == nil || len(overrides.EmbeddedObjectMetaWithAnnotations.Annotations) == 0 {
		return baseMeta.Annotations
	}
	return overrides.EmbeddedObjectMetaWithAnnotations.Annotations
}
...
func (sc *StackContainer) GenerateIngress() (*networking.Ingress, error) {
	...
	hostnames, err := sc.stackHostnames(sc.ingressSpec.Hosts, sc.ingressSpec.StackIngressOverrides)
	if err != nil {
		return nil, err
	}
	if len(hostnames) == 0 {
	...
	// insert annotations
	result.Annotations = mergeLabels(result.Annotations, stackIngressRouteGroupAnnotations(sc.ingressSpec.EmbeddedObjectMetaWithAnnotations, sc.ingressSpec.StackIngressOverrides))
	return result, nil
}

func (sc *StackContainer) GenerateRouteGroup() (*rgv1.RouteGroup, error) {
	if !sc.HasBackendPort() || sc.routeGroupSpec == nil || !sc.routeGroupSpec.StackIngressOverrides.IsEnabled() {
		return nil, nil
	}

	hostnames, err := sc.stackHostnames(sc.routeGroupSpec.Hosts, sc.routeGroupSpec.StackIngressOverrides)
	if err != nil {
		return nil, err
	}
	if len(hostnames) == 0 {
	...
	// insert annotations
	result.Annotations = mergeLabels(result.Annotations, stackIngressRouteGroupAnnotations(sc.routeGroupSpec.EmbeddedObjectMetaWithAnnotations, sc.routeGroupSpec.StackIngressOverrides))
